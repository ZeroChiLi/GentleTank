...
public class AllPropsManager : MonoBehaviour
{
    ...
    public bool spawnAtStart = false;       // 是否在开始时就产生道具
    ...
    private void Start()
    {
        ...
        if (spawnAtStart && propPointDict.Count < maxPropCount)
            SpawnProp();
        Timer.Start();
    }
    ...
    private void SpawnProp()
    {
        ...
        currentProp.OnTouchFinishedEvent += Prop_OnTouchFinishedEvent;  // 添加道具被使用的监听事件
    ...
    private void Prop_OnTouchFinishedEvent(PropBase prop)
    {
        if (propPointDict.ContainsKey(prop))
        {
            propPointDict.Remove(prop);
            prop.OnTouchFinishedEvent -= Prop_OnTouchFinishedEvent;
        }
    }
}
