...
public static class CountDownTimer
{
    ...
    /// <param name="value">起始倒计时值</param>
    /// <param name="interval">时间间隔</param>
    /// <param name="recycle">是否循环（每次计时结束后重新赋值）</param>
    /// <returns>是否倒计时结束</returns>
    static public bool IsCountDownEnd(ObjectPreferences prefs,string key,float value,float interval,bool recycle = false)
    {
        if (!prefs.Contains(key))
            prefs.AddValue(key, value);
